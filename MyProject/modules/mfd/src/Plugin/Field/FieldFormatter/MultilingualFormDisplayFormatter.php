...
class MultilingualFormDisplayFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
   ...
   *   The content translation mangager.
   * @param \Symfony\Component\DependencyInjection\ContainerInterface $container
   *   The container interface.
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, EntityTypeManagerInterface $entity_type_manager, ConfigurableLanguageManager $language_manager, ContentTranslationManagerInterface $manager, ContainerInterface $container) {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if ($this->getSetting('display_translate_table') && $this->account->hasPermission('show multilingual translate table')) {
      ...
      if ($collapsible_state == self::COLLAPSIBLE_STATE_NONE) {
        ...
        $elements = [
          '#type' => 'details',
          '#open' => ($collapsible_state == self::COLLAPSIBLE_STATE_FIRST) || ($collapsible_state == self::COLLAPSIBLE_STATE_ALL_OPEN),
        ];
      }
  ...
  public function overviewTranslationTable(ContentEntityInterface $entity, $entity_type_id = NULL) {
    ...
    $field_ui = ModuleHandler::moduleExists('field_ui') && $this->account->hasPermission('administer ' . $entity_type_id . ' fields');
